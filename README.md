...
| 维度 | 能力 | 证据 |
|------|------|------|
| **Wiki** | LLM 作为编译器驱动 | 7 个 prompt 模板构成编译器管道，raw/ → wiki/ 全自动 |
| | Prompt Engineering 作为工程学科 | 每种 raw 类型对应独立 prompt，可单独调试、替换、A/B 测试 |
| | AI Agent 自主执行 | `/ingest` 管道 7 阶段全自动，URL 进去→结构化知识出来，1 个人工确认点 |
| | 确定性 Guardrails | 6 项纯规则检查，不调 LLM。正则+路径解析+字段校验，硬约束在代码里 |
| | Eval Driven Development | Golden cases + eval runner，compile 末尾自动触发 eval gate |
| | 抽象化循环 | raw/ 碎石路 → prompts/ + templates/ 高速公路 |
| **Quiz** | 摄入即出题 | `/ingest` Stage 5.5 自动从新内容生成复习题到 `questions/` |
| | 交互式测验 | `/review` 选题→渲染 HTML，纸墨质感设计，支持主题筛选、错题回顾 |
| | localStorage 持久化 | 答题进度和错题记录跨 session 保存，下次可"错题复习" |
...
```
                     Claude Code (编译器驱动)
                           │
raw/ ──→ [prompts/ 7个编译器Pass] ──→ wiki/ ──→ health-check.ps1
  │                                      │            │
  │    questions/ ←── ingest auto-gen ──┘      6项确定性检查
  │         │
  │    .claude/skills/review/
  │         │
  └──── output/quiz-*.html (交互刷题)
```
...
prompts/                          # 7 个 LLM 编译器 Pass（可替换可A/B）
